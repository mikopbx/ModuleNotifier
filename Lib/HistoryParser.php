...
class HistoryParser
{
    ...
    public static function getCdr(array $filter = []): array
    {
        ...
        if (!isset($filter['columns'])) {
            $filter['columns'] = 'id,start,answer,src_num,dst_num,dst_chan,endtime,linkedid,recordingfile,dialstatus,UNIQUEID,did';
        }
            ...
            foreach ($cdrData as $cdr){
                ...
                if(!isset($resultRows[$cdr['linkedid']])){
                    ...
                    if(($srcInner && !$dstInner) || (stripos( $cdr['src_chan'], 'local/') !== false
                        ...
                        $line = $cdr['to_account'];
                        $resultRows[$cdr['linkedid']]['src'] = $cdr['src_num'];
                        $resultRows[$cdr['linkedid']]['dst'] = $cdr['dst_num'];
                        ...
                        $resultRows[$cdr['linkedid']]['src'] = $cdr['src_num'];
                        $resultRows[$cdr['linkedid']]['dst'] = empty($cdr['dst_num'])?$cdr['did']:$cdr['dst_num'];
                    ...
                    $resultRows[$cdr['linkedid']]['did'] = $cdr['did']??'';
                    $resultRows[$cdr['linkedid']]['line']     = $line;
                }
                if( (int)$cdr['is_app'] !== 1 && (int)$cdr['billsec'] !== 0 ){
