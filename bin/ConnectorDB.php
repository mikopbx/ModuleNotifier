...
class ConnectorDB extends WorkerBase
{
    ...
    {
        $this->logger   = new Logger('ConnectorDB', 'ModuleNotifier');
    ...
    public function updateSettings(int $newCdrOffset=0):void
    {
        ...
        if(empty($settings->referenceDate)){
            $settings->referenceDate = date("Y-m-d H:i:s.0", strtotime("-1 days"));
            $settings->save();
        }
        if(empty($settings->cdrOffset)){

            $filter = [
                'limit' => 1,
                'order' => 'id DESC'
            ];
            $data = HistoryParser::getCdr($filter);
            $settings->cdrOffset = $data[0]['id']??1;
            $settings->save();
            $this->logger->writeInfo('Update cdrOffset from cdr: '.$settings->cdrOffset.'...');
        }
        ...
        foreach ($providers as $provider) {
            $this->providerName[$provider->uniqid] = $provider->description;
        }
    }
    ...
    private function saveResultInTmpFile($data):string
    {
        ...
        $tmoDirName = $dirsConfig->path('core.tempDir') . '/ModuleNotifier';
    ...
    public function syncCdrData():void
    {
        ...
        foreach ($cdrData as $key => $cdr){
            ...
            foreach ($cdr['rows'] as $row){
                ...
                $dbData->save();
                $this->sendAudioToTelegram($row);
                unset($dbData);
            }
        }
        if($oldOffset !== $this->cdrOffset){
            $this->logger->writeInfo("Update cdrOffset from $oldOffset to $this->cdrOffset ");
    ...
    public function sendEditMessage($cdr):array
    {
        $this->logger->writeInfo('sendEditMessage: '.json_encode($cdr));

        $messageId = '';
        $data = MessageData::findFirst(["linkedId=:linkedId:", 'bind' => ['linkedId' => $cdr['linkedid']]]);
        if($data){
            $messageId.= $data->messageId;
            $this->logger->writeInfo('find message data: '.json_encode($data->toArray()));
        }
        $response = $this->sendCallMessageToTelegram($cdr, $messageId);
        if(!$data){
            $this->logger->writeInfo('add new message data: '.json_encode($response->data));
            $data = new MessageData();
            $data->linkedId = $cdr['linkedid'];
            $data->messageId = $response->data['result']['message_id'];
            $data->messageText = $response->data['result']['text'];
            $data->save();
        }
    ...
    private function sendAudioToTelegram($dbData)
    {
        if($dbData['typeCall'] === CallHistory::CALL_TYPE_INNER){
            return;
        }
        if(!file_exists($dbData['recordingfile'])){
            return;
        }
        $messageId = 0;
        $data = MessageData::findFirst(["linkedId=:linkedId:", 'bind' => ['linkedId' => $dbData['linkedid']]]);
        if($data){
            $messageId = $data->messageId;
        }
